...
const itemCheck = (element) => element ===item
  return callback(list.some(itemCheck))
}
...
function firstNamesAllCaps(runners) {
  let capFirstName = runners.map(item => `${item.first_name}`.toUpperCase())
  return capFirstName
}
/**
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const filteredRunner = runners.filter(function(item){
...
const donations = runners.reduce(function(a, b) {
  return a + b.donation;
}, 0)
return donations
}
